...
```{r fixed-frame, echo = FALSE, fig.width = 6, fig.height = 4, fig.cap="The fixed variable framework where the sample scores in matrix ${\\bf{Z}}$ are sliced according to the levels of the time variable $T$ and the variable vectors in matrix ${\\bf{V}}$ remaining fixed."}
...
From the biplot in Figure \@ref(fig:biplot-Afr), the relationships between the variables are exposed, but there is some difficulty to separate the regions without intervention. Importantly, information on the time differences are not available which means a sequential exploration is impossible. These aspects are are addressed in \CRANpkg{moveEZ}. 
