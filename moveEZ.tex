...
\begin{figure}[!]
\includegraphics[width=1\linewidth]{figures/Fixed_variable_frame} \caption{The fixed variable framework where the sample scores in matrix ${\bf{Z}}$ are sliced according to the levels of the time variable $T$ and the variable vectors in matrix ${\bf{V}}$ remaining fixed.}\label{fig:fixed-frame}
\end{figure}
...
Because \texttt{moveplot2()} fits a separate PCA per time slice, it requires a sufficient number of observations within each slice to perform the decomposition. It is therefore not suitable for datasets with a single observation per group per time level, such as the aggregated \emph{gapminder} data used in Section \ref{gapminderdata}. The \texttt{Africa\_climate} dataset, which contains multiple observations per region and time level, is used to demonstrate this function throughout this section.
